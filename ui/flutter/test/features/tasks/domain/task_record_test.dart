...
import 'package:ponydownloader/api/model/task.dart' as api;
...
void main() {
  ...
  test('file names preserve hash and question mark characters for type detection', () {
    expect(
      ...
      PonyDownloaderIcons.fileVideo,
    );
    expect(taskFileTypeIcon('What is this? #2.MP4'), PonyDownloaderIcons.fileVideo);
    expect(taskFileTypeIcon('Episode #2.mp3'), PonyDownloaderIcons.fileAudio);
  });

  test('URL fallback ignores query and fragment when detecting file type', () {
    expect(taskFileTypeIcon('https://example.com/video.mp4?token=abc#part'), PonyDownloaderIcons.fileVideo);
    expect(taskFileTypeIcon('https://example.com/download?name=video.mp4'), PonyDownloaderIcons.file);
    expect(taskFileTypeIcon('Playlist #2.mp4', isFolder: true), PonyDownloaderIcons.folder);
  });
}
