...
此目录是 PonyDownloader 文件分类图标的唯一源物料目录。它位于 Flutter 工程内，但只用于设计和生成流程，不得加入 `pubspec.yaml` 的 `assets` 配置。

## 维护限制

- 不要绕过 `$gopeed-file-category-icons` 技能直接修改、移动或批量替换这里的 SVG 与 manifest。
- 需要调整图标时，必须配套使用该技能的安全区、码位兼容、字体生成和多尺寸预览流程。
- 不要直接编辑生成的 `PonyDownloaderIcons.ttf` 或 `ponydownloader_icons.dart`。
...
- Flutter 字体：`assets/fonts/PonyDownloaderIcons.ttf`
- Flutter 常量：`lib/core/icons/ponydownloader_icons.dart`
