...
@objcMembers
final class PonyDownloaderLiveActivityManager: NSObject {

    static let shared = PonyDownloaderLiveActivityManager()

    // PonyDownloader emits progress approximately every 350 ms.
    ...
    private func generationForProgress(
        ...
        // refreshActivity still verifies that PonyDownloader reports the task
    ...
    // MARK: - PonyDownloader event entry point
    ...
    // MARK: - Async PonyDownloader invocation

    private func invokePonyDownloader(
        method: String,
        path: String,
        query: String = "",
        body: String = ""
        ...
        await withCheckedContinuation {
            ...
            PonyDownloaderInvokeAsyncNative(
                method,
                path,
                query,
                body
    ...
    // MARK: - PonyDownloader runtime status
    ...
    private func getRuntimeStatus(
        ...
        guard code == 0 else {
            let message =
                root["msg"] as? String
                ?? "Unknown PonyDownloader API error"

            print(
                "LiveActivity: PonyDownloader status error:",
                message
            )
    ...
    private func makeState(
    ...
    ) -> PonyDownloaderDownloadAttributes.ContentState {
        ...
        guard
            ...
            return PonyDownloaderDownloadAttributes
                .ContentState(
                    progress: progress,
    ...
    private func findActivities(
        taskID: String
    ) -> [Activity<PonyDownloaderDownloadAttributes>] {
        return Activity<
            PonyDownloaderDownloadAttributes
        >
        .activities
        .filter {
    ...
    private func collapseDuplicateActivities(
        _ activities: [Activity<PonyDownloaderDownloadAttributes>],
        content: ActivityContent<PonyDownloaderDownloadAttributes.ContentState>
    ) async {
